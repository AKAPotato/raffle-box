...
# 2. 网页上切到「B站评论抽奖」模块，粘贴视频链接（BV 号或完整 URL 均可）
...
- 推送成功后，脚本会**自动为当前版本号创建并推送 tag**（如 `v1.0.1`）。注意：**代码推送 ≠ 发布版本** —— GitHub 上的「Release」仍需手动点一次：进入仓库 Releases 页 → `Create a new release` → 选择对应 tag → 正文直接复制 `CHANGELOG.md` 里该版本的条目。
